...
/// Helper macro for printing error log for index out of range.
#define logOutOfRange(Err, Cate, N, Bound)                                     \
  spdlog::error(ErrCode::Err);                                                 \
  spdlog::error(ErrInfo::InfoForbidIndex(ErrInfo::IndexCategory::Cate,         \
                                         static_cast<uint32_t>(N),             \
                                         static_cast<uint32_t>(Bound)));       \
  return Unexpect(ErrCode::Err);
...
Expect<void> FormChecker::checkInstr(const AST::Instruction &Instr) {
  ...
  auto checkBlockType = [this](std::vector<VType> &Buffer,
    ...
    if (BType.IsValType) {
      ...
      if (TypeIdx >= Types.size()) {
        logOutOfRange(InvalidFuncTypeIdx, FunctionType, TypeIdx, Types.size());
      }
  ...
  auto checkCtrlStackDepth = [this](uint32_t N) -> Expect<uint32_t> {
    ...
    if (N >= CtrlStack.size()) {
      /// Branch out of stack.
      logOutOfRange(InvalidLabelIdx, Label, N, CtrlStack.size());
    }
  ...
  auto checkMemAndTrans = [this,
    ...
    if (Instr.getTargetIndex() >= Mems) {
      logOutOfRange(InvalidMemoryIdx, Memory, Instr.getTargetIndex(), Mems);
    }
  ...
  auto checkLaneAndTrans = [this,
    ...
    if (Instr.getMemoryLane() >= N) {
      logOutOfRange(InvalidLaneIdx, Lane, Instr.getMemoryLane(), N);
    }
    return StackTrans(Take, Put);
  };
  ...
  case OpCode::Call: {
    ...
    if (N >= Funcs.size()) {
      logOutOfRange(InvalidFuncIdx, Function, N, Funcs.size());
    }
  ...
  case OpCode::Call_indirect: {
    ...
    if (T >= Tables.size()) {
      logOutOfRange(InvalidTableIdx, Table, T, Tables.size());
    }
    ...
    if (N >= Types.size()) {
      logOutOfRange(InvalidFuncTypeIdx, FunctionType, N, Types.size());
    }
    if (auto Res = popType(VType::I32); !Res) {
      return Unexpect(Res);
    }
  ...
  case OpCode::Local__tee: {
    if (Instr.getTargetIndex() >= Locals.size()) {
      logOutOfRange(InvalidLocalIdx, Local, Instr.getTargetIndex(),
                    Locals.size());
    }
  ...
  case OpCode::Global__get: {
    if (Instr.getTargetIndex() >= Globals.size()) {
      logOutOfRange(InvalidGlobalIdx, Global, Instr.getTargetIndex(),
                    Globals.size());
    }
  ...
  case OpCode::Table__copy: {
    ...
    if (Instr.getTargetIndex() >= Tables.size()) {
      logOutOfRange(InvalidTableIdx, Table, Instr.getTargetIndex(),
                    Tables.size());
    }
    ...
    if (Instr.getOpCode() == OpCode::Table__get) {
      ...
      if (Instr.getSourceIndex() >= Elems.size()) {
        logOutOfRange(InvalidElemIdx, Element, Instr.getSourceIndex(),
                      Elems.size());
      }
      ...
      if (Instr.getSourceIndex() >= Tables.size()) {
        logOutOfRange(InvalidTableIdx, Table, Instr.getSourceIndex(),
                      Tables.size());
      }
      /// Check is the reference types matched.
  ...
  case OpCode::Elem__drop:
    ...
    if (Instr.getTargetIndex() >= Elems.size()) {
      logOutOfRange(InvalidElemIdx, Element, Instr.getTargetIndex(),
                    Elems.size());
    }
    return {};
  ...
  case OpCode::Memory__size:
    return checkMemAndTrans({}, std::array{VType::I32});
  case OpCode::Memory__grow:
    return checkMemAndTrans(std::array{VType::I32}, std::array{VType::I32});
  case OpCode::Memory__init:
    ...
    if (Instr.getSourceIndex() >= Datas.size()) {
      logOutOfRange(InvalidDataIdx, Data, Instr.getSourceIndex(), Datas.size());
    }
    return StackTrans(std::array{VType::I32, VType::I32, VType::I32}, {});
  case OpCode::Memory__copy:
    /// Check the source memory index.
    if (Instr.getSourceIndex() >= Mems) {
      logOutOfRange(InvalidMemoryIdx, Memory, Instr.getSourceIndex(), Mems);
    }
    [[fallthrough]];
  case OpCode::Memory__fill:
    return checkMemAndTrans(std::array{VType::I32, VType::I32, VType::I32}, {});
  case OpCode::Data__drop:
    /// Check the target data index.
    if (Instr.getTargetIndex() >= Datas.size()) {
      logOutOfRange(InvalidDataIdx, Data, Instr.getTargetIndex(), Datas.size());
    }
    return {};
  ...
  case OpCode::V128__load8_lane:
    return checkAlignAndTrans(8, std::array{VType::I32, VType::V128},
                              std::array{VType::V128}, true);
  case OpCode::V128__load16_lane:
    return checkAlignAndTrans(16, std::array{VType::I32, VType::V128},
                              std::array{VType::V128}, true);
  case OpCode::V128__load32_lane:
    return checkAlignAndTrans(32, std::array{VType::I32, VType::V128},
                              std::array{VType::V128}, true);
  case OpCode::V128__load64_lane:
    return checkAlignAndTrans(64, std::array{VType::I32, VType::V128},
                              std::array{VType::V128}, true);
  case OpCode::V128__store8_lane:
    return checkAlignAndTrans(8, std::array{VType::I32, VType::V128}, {}, true);
